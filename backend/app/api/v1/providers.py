...
async def get_providers(
    ...
    - **limit**: 返回数量上限
    """
    providers = await llm_provider.get_multi(db, skip=skip, limit=limit)
    return providers
...
async def get_provider(
    ...
    if not provider:
        raise HTTPException(
            ...
            detail=f"ID为 {provider_id} 的服务商不存在"
        )
    return provider
...
async def create_provider(
    ...
    """
    # 检查名称是否已存在
    existing_provider = await llm_provider.get_by_name(db, name=provider_in.name)
    if existing_provider:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"名称为 {provider_in.name} 的服务商已存在"
        )
